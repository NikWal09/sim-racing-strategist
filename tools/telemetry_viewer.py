#!/usr/bin/env python3
"""Generator samodzielnej strony HTML do odtwarzania telemetrii (Garage 61).

Skanuje katalog z nagraniami (pliki JSON tworzone przez TelemetryRecorder),
grupuje okrazenia po torze i wpisuje wszystkie dane do jednego, samodzielnego
pliku HTML. Strona dziala offline (zwykle otwarcie w przegladarce - bez serwera,
bez internetu, bez CDN): rysuje mape toru ("nitke") kolorowana predkoscia oraz
OSOBNE wykresy predkosci, gazu, hamulca, biegu, obrotow i kata kierownicy.
Mozna wybrac kilka okrazen naraz i porownac je na jednym wykresie.

Analiza fragmentu toru (styl Garage 61): przeciagniecie myszka po dowolnym
wykresie przybliza wybrany odcinek (wszystkie wykresy i mapa pokazuja tylko
ten fragment, mapa podswietla go na tle przygaszonego toru), przyciski
S1/S2/S3 wybieraja tercje okrazenia, a "Cale okrazenie" wraca do calosci.
Dla fragmentu liczony jest czas przejazdu kazdego zaznaczonego okrazenia.

Porownywanie okrazen: przy >= 2 zaznaczonych okrazeniach pojawia sie wykres
DELTY - ile czasu kazde okrazenie traci/zyskuje do najszybszego z zaznaczonych
w kazdym punkcie toru. Najechanie myszka na dowolny wykres rysuje pionowy
KURSOR zsynchronizowany na wszystkich wykresach (plakietki z wartosciami
kazdego okrazenia + dystans w metrach) i znacznik pozycji na mapie toru.

Uzycie:
    python tools/telemetry_viewer.py
    python tools/telemetry_viewer.py --recordings recordings --out recordings/telemetria.html
"""
...
# Baza nazw aut GT7 (kod -> nazwa). Import jest opcjonalny - skrypt dziala
# takze uruchomiony z innego katalogu, wtedy zostaja surowe kody aut.
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
try:
    from gt7_engineer.data import car_name as _car_name
except Exception:  # noqa: BLE001
    _car_name = None


def load_laps(recordings_dir: str) -> list[dict]:
    """Wczytuje wszystkie pliki okrazen z katalogu i sortuje je po czasie kolka.

    Starszym nagraniom (bez pola car_name) dokladamy nazwe auta z bazy.
    """
    laps: list[dict] = []
    for path in sorted(glob.glob(os.path.join(recordings_dir, "*.json"))):
        ...
        if not data.get("car_name") and _car_name is not None:
            data["car_name"] = _car_name(data.get("car_code"))
        laps.append(data)
    laps.sort(key=lambda d: (d.get("track_key", ""), d.get("lap_ms", 0)))
    return laps


def build_html(laps: list[dict]) -> str:
    payload = json.dumps(laps, ensure_ascii=False, separators=(",", ":"))
...
LAPS.forEach((lap, i) => {
  ...
  lap._ch = ch;
  // dystans skumulowany w plaszczyznie x-z (do osi X wykresow i mapy)
...
// Grupowanie po torze: klastrujemy okrazenia po fingerprincie (dlugosc + obrys)
// z tolerancja, zamiast porownywac sztywny track_key. Dzieki temu okrazenia z
// tego samego toru trafiaja razem nawet, gdy dlugosc rozni sie o kilka metrow
// (rozna linia jazdy) - bez ryzyka, ze wpadna po dwoch stronach progu kwantyzacji.
...
let curTrack = null;        // id grupy (toru)
const selected = new Set();   // indeksy wybranych okrazen
...
function setTrack(id) {
  curTrack = id;
  selected.clear();
  view = { a: 0, b: 1 };   // zmiana toru = wroc do pelnego okrazenia
  const laps = lapsForTrack(id);
  if (laps.length) selected.add(laps[0]._idx);  // domyslnie najszybsze
  renderLapList();
  renderMain();
}
...
// --- zakres widoku: fragment toru (ulamek dystansu okrazenia) ---
// Przeciagniecie myszka po dowolnym wykresie przybliza wybrany odcinek toru
// (jak w Garage 61). Przyciski S1/S2/S3 wybieraja tercje, a "Cale okrazenie"
// wraca do pelnego widoku. Mapa podswietla wybrany fragment.
let view = { a: 0, b: 1 };
const CHARTS = [];          // funkcje przerysowujace wykresy (resize/overlay)
const CHART_PAD = { l: 38, r: 8 };

function setView(a, b) {
  a = Math.max(0, Math.min(a, b));
  b = Math.min(1, Math.max(a, b));
  if (b - a < 0.005) return;          // za waski zakres - ignoruj
  view = { a: a, b: b };
  renderMain();
}
...
// Czas przejazdu wybranego fragmentu dla okrazenia (z kanalu t).
...
// --- kursor: pionowa linia zsynchronizowana na wszystkich wykresach ---
// Najechanie myszka na wykres ustawia kursor w tym samym punkcie toru na
// kazdym wykresie (plakietki z wartosciami) i znacznik pozycji na mapie -
// jak w Garage 61. Przeciagniecie nadal zaznacza fragment.
let cursorF = null;            // pozycja kursora (ulamek dystansu okrazenia)
...
// Interpolacja po dystansie: indeks probki i waga dla ulamka okrazenia f.
...
// Najszybsze z zaznaczonych okrazen - referencja wykresu delty.
function fastestSelected() {
  const sel = [...selected].map(i => LAPS[i]);
  return sel.reduce((a, b) => (b.lap_ms < a.lap_ms ? b : a), sel[0]);
}

// Czy KAZDE zaznaczone okrazenie ma dany kanal (starsze nagrania maja mniej).
...
function speedColor(v, vmin, vmax) {
  ...
  // niski = niebieski, sredni = zielony, wysoki = czerwony
  const h = (1 - t) * 220;   // 220 (nieb.) -> 0 (czerw.)
  return "hsl(" + h + ",80%,55%)";
}
...
function drawMap() {
  ...
  sel.forEach(lap => {
    ...
    lap.samples.forEach((s, j) => {
      ...
      // Skala kolorow predkosci - tylko z widocznego fragmentu.
      if (inView(lap, j)) {
  ...
  const py = z => h - (offZ + (z-minZ)*sc);   // odbicie osi Z (canvas Y w dol)
  const single = sel.length === 1;

  // 1) Caly tor przygaszony (kontekst polozenia fragmentu).
  if (!isFullView()) {
    sel.forEach(lap => {
  ...
  // 2) Wybrany fragment (lub calosc) w kolorze.
  sel.forEach(lap => {
    const ch = lap._ch, S = lap.samples;
    if (single) {
  ...
  // Znacznik kursora: pozycja auta na torze w punkcie wskazanym na wykresach.
...
// Czasy przejazdu wybranego fragmentu (per zaznaczone okrazenie).
...
// Rysuje wykres kanalu wzgledem ulamka dystansu okrazenia, ale tylko w oknie
// [view.a, view.b] - dzieki temu zaznaczenie fragmentu "przybliza" wykresy.
// Punkty spoza okna sa przycinane clipem (linia pozostaje ciagla na krawedzi).
function lineChart(id, getY, yMin, yMax, opts) {
  ...
  // Kursor: pionowa linia + plakietki z wartoscia kazdego okrazenia w tym
  // punkcie toru (interpolacja po dystansie) + dystans w metrach na dole.
...
// Zaznaczanie fragmentu myszka: przeciagniecie po wykresie -> setView(zakres).
// Podczas przeciagania rysujemy polprzezroczysta nakladke (jak w Garage 61).
function attachSelect(c, redraw) {
  ...
  c.onmousemove = e => {
    if (x0 === null) {
      // Bez przeciagania: kursor zsynchronizowany na wszystkich wykresach.
  ...
  c.onmouseup = e => {
    ...
    if (Math.abs(x1 - x0) > 8) {
      ...
      redraw();   // klik bez przeciagania - tylko sprzatamy nakladke
    }
    x0 = null;
  };
...
function drawAllCharts() {
  ...
  // Delta do najszybszego z zaznaczonych: ile czasu kazde okrazenie traci
  // (dodatnia) lub zyskuje (ujemna) dojezdzajac do tego samego punktu toru.
  // Porownanie po ulamku dystansu - jak pozostale wykresy.
  if (sel.length >= 2) {
  ...
  // Paliwo [%] - diagnostyka spalania (tylko nowe nagrania z kanalem fuel_pct).
  // Os Y dopasowana do faktycznego zakresu, by widac bylo zuzycie w trakcie
  // kolka i ewentualny skok po tankowaniu.
  if (selHas("fuel_pct")) {
    let fmin = 100, fmax = 0;
    sel.forEach(l => l.samples.forEach(s => {
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Generator strony HTML z telemetria GT7.")
    ap.add_argument("--recordings", default="recordings", help="katalog z plikami okrazen (.json)")
    ap.add_argument("--out", default=None, help="plik wyjsciowy HTML (domyslnie <recordings>/telemetria.html)")
    args = ap.parse_args()

    rec_dir = args.recordings
    if not os.path.isdir(rec_dir):
        print(f"Brak katalogu z nagraniami: {rec_dir}")
        return 1

    laps = load_laps(rec_dir)
    out = args.out or os.path.join(rec_dir, "telemetria.html")
    html = build_html(laps)
    with open(out, "w", encoding="utf-8") as f:
        f.write(html)

    print(f"Zapisano {len(laps)} okrazen do: {out}")
    if not laps:
        print("(Katalog jest pusty - pojezdz w GT7 z wlaczonym nagrywaniem.)")
    else:
        keys = sorted({l.get('track_key', '?') for l in laps})
        print(f"Tory: {', '.join(keys)}")
    return 0
